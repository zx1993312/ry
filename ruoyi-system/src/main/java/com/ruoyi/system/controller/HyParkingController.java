...
public class HyParkingController extends BaseController {
    ...
    {
        ExcelUtil<HyParkingInf> util = new ExcelUtil<HyParkingInf>(HyParkingInf.class);
        List<HyParkingInf> userList = util.importExcel(file.getInputStream());
        String operName = ShiroUtils.getSysUser().getLoginName();
        String message = hyParkingInfService.importOwnerRegistration(userList, updateSupport, operName);
	...
	@RequiresPermissions("system:set:view")
	 ...
	 public AjaxResult importTemplate() {
	  ExcelUtil<HyParkingInf> util = new ExcelUtil<HyParkingInf>(HyParkingInf.class);
	  return util.importTemplateExcel("车位车辆管理");
	 }
}
